...
def get_db_info(site_host=None, envir=None):
    if envir == 'pro':
        query = db.query(SiteDatabase).filter(
            and_(~SiteDatabase.site_host.like('%preview%'), ~SiteDatabase.site_host.like('%test%'))).all()
    elif envir == 'dev':
        query = db.query(SiteDatabase).filter(SiteDatabase.site_host.like('%preview%')).all()
    else:
        if site_host:
            query = db.query(SiteDatabase).filter_by(site_host=site_host).first()
        else:
            query = db.query(SiteDatabase).all()
